...
def heatmap(
    ...
    if len(xs_shot_plot) > 0:
        ...
        ax.legend()
        ax.set_title("Pass heatmap + shot spatial position")
    else:
...
def expected_threat(player) -> float:
    ...
    """
    player_event = st.session_state.event_data[
        st.session_state.event_data["player_id"].astype(int) == int(player.player_id)
    ]

    # If player has no events, return 0
    if player_event.empty:
        return 0.0

    # If xT column exists, sum it; otherwise estimate 0.02 per event
    if "xthreat" in player_event.columns:
        xt_sum = player_event["xthreat"].astype(float).sum()
        print("expected threat computed ")

    else:
        xt_sum = len(player_event) * 0.02
        print("expected threat set to zero")
    return xt_sum
...
DEFENSIVE_END_TYPES = ["pressing","presure","recovery_press",
...
TEAM_colors = {
  ...
  "Wellington Phoenix FC": "#F2D200",
  "Brisbane Roar FC": "#D56E08",
  "Perth Glory Football Club": "#5E2D91",
  "Melbourne City": "#6CADDF",
  "Central Coast Mariners Football Club": "#F1C40F",
  "Adelaide United Football Club": "#CE1C24",
  "Sydney Football Club": "#60AFFF",
  "Melbourne Victory Football Club": "#000942",
  "Macarthur FC": "#C4954C",
  "Western United": "#007C41"
}
